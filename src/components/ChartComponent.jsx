...
import { getTodayDateString, getDatesInRange } from '../utils/dateUtils'; // getDatesInRange is now exported
...
const ChartComponent = ({ checkins }) => {
    ...
    const data = useMemo(() => {
        const rewardCollectedMap = new Map();
        checkins.forEach(c => {
            if (c.rewardCollected) { // Check if reward was collected
                rewardCollectedMap.set(c.date, true); // Mark date as reward collected
            }
        });

        // Map dates to 1 (reward collected) or 0 (reward not collected) for the chart
        const chartData = dates.map(date => rewardCollectedMap.has(date) ? 1 : 0);
        ...
        return {
            ...
            datasets: [
                {
                    label: 'Daily Reward Collection', // Label for the dataset
                    data: chartData,
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Last 30 Days Reward Collection', // Updated title
                font: {
                    size: 14,
                    weight: 'bold'
                },
            ...
            tooltip: {
                callbacks: {
                    label: function(context) {
                        return context.raw === 1 ? 'Reward Collected' : 'No Reward'; // Updated tooltip
                    }
                }
            }
        },
            ...
            y: {
                ...
                ticks: {
                    ...
                    callback: function(value) {
                        return value === 1 ? 'Collected' : 'Missed'; // Custom labels for Y-axis
                    }
                },
                grid: {
